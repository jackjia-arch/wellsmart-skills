...
2. 点 **Add marketplace（添加插件市场）**，填仓库地址：`GITHUB_OWNER/wellsmart-skills`（或完整地址 `https://github.com/GITHUB_OWNER/wellsmart-skills`），确定。
...
```
/plugin marketplace add GITHUB_OWNER/wellsmart-skills
...
（把上面的 `GITHUB_OWNER` 换成公司仓库所在的 GitHub 用户名或组织名。）
