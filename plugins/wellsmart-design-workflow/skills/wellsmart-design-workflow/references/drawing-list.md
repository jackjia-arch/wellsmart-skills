...
- `revision` is the ProjectBook `revision_id` of that sheet (`references/project-knowledge-base.md`) — a catalogue fact, not a label printed on a drawing that is still in draft (date only on the sheet itself). The matrix is regenerated whenever a sheet is revised, so the matrix of a frozen package names the exact revisions the package contains.
