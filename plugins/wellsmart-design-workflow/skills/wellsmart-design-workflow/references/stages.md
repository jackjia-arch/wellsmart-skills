...
**Outputs.** Brief · Site Data Pack index (have / missing / to commission) · Regulatory Basis · Certification Pathway · Programme · Cost Plan 0 · specialist matrix · handover matrix · ProjectBook project · version lock.
...
**AI does.** Generate IFC from the DB with IfcOpenShell: ARCH + STR + ID at LOD 300; MEP only as space reservations (riser volumes, plant rooms, main service corridors). Model-check scripts: storeys, grids, naming, units, coordinate system, classification (BIMForum LOD, ISO 19650 naming, Uniclass). Publish the release to the ProjectBook, whose 3D viewer lets the PD walk through and pick issues; Blender + Bonsai available to technical staff for offline checking. `references/ifc-review.md`.
...
| S4 | IFC 3D (ARCH + STR + ID, LOD 300) | AI, IfcOpenShell script | ProjectBook 3D viewer for walk / section / pick; GUID, coordinates, viewpoint, snapshot and release saved automatically; nobody enters Revit; Blender + Bonsai only assists checking |
