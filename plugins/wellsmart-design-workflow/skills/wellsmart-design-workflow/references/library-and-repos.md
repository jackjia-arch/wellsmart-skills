...
Where the project repository lives: the working files sit in the shared Google Drive `Projects` sync folder, so Cowork (through the Drive connector or the linked computer) and Claude Code (local shell) see the same files; git history is kept by whoever runs Claude Code on the folder (the `.git` directory is never synced through Drive; staff never run git). Formal records (every stage output, revision and endorsement) are registered in the Project Book by ID (`references/project-knowledge-base.md`, `references/project-book.md`); the Drive folder is the working copy, not the record.
