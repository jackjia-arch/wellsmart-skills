# UI guidance — building or judging any project page or workbench screen

Part II of the handbook (UI Guidance 1.0) defines the product interface for design, review, procurement, the knowledge base and operations: Apple-like restraint, hierarchy and familiarity. This file is what the AI hands to a developer and what it checks a page against. The handbook's own demo screens are static examples; nothing in them is connected to a real project or backend, and every demo value is labelled DEMO.
...
>
> Add IFC 3D review with That Open Components + Fragments: picking saves both IFC GlobalIds, DB IDs, engineering coordinates in metres, the coordinate-transform snapshot, camera / clipping / snapshot and release / hash; issues persist, the ID mapping is kept across releases, and a missing component never auto-closes an issue. The AI edits source data with IfcOpenShell; Revit production detailing is entirely manual. Deployment uses only the Netlify portal app + company Runner / API + PostgreSQL catalogue + company AWS S3 originals. IT publishes the program with the official Netlify connector; each project has a fixed project_id and original URL, and staff uploads of new content do not redeploy the site. Unified ADD / REVISE / ATTACH ingestion with stable document_id and immutable revision / hash; the merge summary lists added / revised / attached / retained / deleted, and unsubmitted content is retained. Large files upload directly to S3; integrity verification and model conversion run asynchronously; a failure does not change the current effective revision. Mutable current / catalogue / search responses must not be cached by browser or CDN; on reopen or focus regain check catalog_version. Repeated submission of the same logical item is idempotent; content-hash deduplication must not skip new attachments or asset relations. Do not show infrastructure nouns in the staff interface.
>
