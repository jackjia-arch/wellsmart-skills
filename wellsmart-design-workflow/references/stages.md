...
**Outputs.** Brief · Site Data Pack index (have / missing / to commission) · Regulatory Basis · Certification Pathway · Programme · Cost Plan 0 · specialist matrix · handover matrix · catalogue skeleton · version lock.
...
**Specialist freeze items at S3.** Fire engineering: strategy finalised, egress calculation package, PBDB / FEB draft prepared by the AI (objectives, methods, acceptance criteria, consultation required). Wet fire: schematic, preliminary hydraulics, effective storage, pump duty, pressure zones, diesel / power / ventilation interfaces. Dry fire: detection / EWIS zoning, draft interlocks with MEP / lifts / access control. DDA: continuous route, ramps, rooms and toilets at 1:50 with clear dimensions. Façade: joint thermal + acoustic + wind + inter-storey movement option comparison and wall section. Acoustics: wall / glazing build-ups, plant positions, flanking control. Traffic: swept paths, ramp long-sections, vehicle headroom and waste flow.
...
**AI does.** Generate IFC from the DB with IfcOpenShell: ARCH + STR + ID at LOD 300; MEP only as space reservations (riser volumes, plant rooms, main service corridors). Model-check scripts: storeys, grids, naming, units, coordinate system, classification (BIMForum LOD, ISO 19650 naming, Uniclass). Publish the That Open web viewer release for the PD to walk through and pick issues; Blender + Bonsai available to technical staff for offline checking. `references/ifc-review.md`.
...
**Coordination.** Corridor / plant-room 2D coordination sections from the DB; ceiling-zone stacking rule (structure → fire → ducts → pipes → trays); crossing pre-check script.
...
**Web issue loop.** People walk and section the model per level / discipline in the HTML viewer and pick components to raise clash / clearance / maintenance issues; coordinates, both GUIDs, snapshot, viewpoint and release are recorded automatically. The AI reads confirmed issues against the frozen IFC / DB, changes source parameters, reruns only the affected calcs / sheets / BQ, regenerates the IFC, reruns clash, publishes a new release marked READY_FOR_REVIEW; a person or independent review closes. Full contract: `references/ifc-review.md`.
...
| S4 | IFC 3D (ARCH + STR + ID, LOD 300) | AI, IfcOpenShell script | That Open viewer for walk / section / pick; GUID, coordinates, viewpoint, snapshot and release saved automatically; nobody enters Revit; Blender + Bonsai only assists checking |
